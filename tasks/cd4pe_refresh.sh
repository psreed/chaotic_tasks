...
if [ -z $IMG ]; then echo "Could not get image ID for puppet/continuous-delivery-for-puppet-enterprise."; echo $IMG; exit 1; fi

CON=`docker container list | awk '/puppet\/continuous-delivery-for-puppet-enterprise/ && /\/sbin\/init/ && /cd4pe/ {print $1}'`
if [ -z $CON ]; then echo "Could not get container ID for puppet/continuous-delivery-for-puppet-enterprise."; echo $CON; exit 1; fi
